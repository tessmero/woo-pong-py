 ...
 */
export function exportLutAsNpy(lut: Lut, filePath: string) {
...
/**
 * @file lut-exporter-py.ts
 *
 * Exports LUT data as a Python object (dict of dicts or nested lists) for use in Python.
 * Provides a function to serialize a LUT instance to a Python file/string.
 *
 * Usage:
 *   import { exportLutAsPython } from './lut-exporter-py'
 *   const pyString = exportLutAsPython(lutInstance, 'MyLut')
 *   // Write pyString to a .py file
 */

import type { Lut, LeafValues } from '../src/simulation/luts/lut'
 ...
 */
function toPyLiteral(val: unknown): string {
 ...
 */
export function exportLutAsPython(lut: Lut, pyName: string, asDict = false): string {
  // Gather all leaf values
  const depth = lut.reg.depth
  const detail = lut.detail
  const indices: number[][] = []
  function gather(idx: number[], d: number) {
    if (d === depth) {
      indices.push([...idx])
      return
    }
  ...
  const flatData: number[] = Array.from(lut.data);
  const shape = detail;
