...
import { BIG_CIRCLE_RADIUS, N_GEAR_FRAMES, N_GEAR_TEETH } from 'simulation/gear-constants'
...
export class TwoGears extends RoomLayout {
  ...
  computePositions(): Array<[number, Vec2]> {
    ...
    const m = 1.385
    const gearSpacing = 2 * BIG_CIRCLE_RADIUS * m // center-to-center distance between adjacent gears
    const middleYOffset = -BIG_CIRCLE_RADIUS * 1 // vertical offset for the middle gear
    ...
    // this.gears.push(this._buildGear(leftPos, 'counter-clockwise', leftFrame))
    // this.gears.push(this._buildGear(middlePos, 'clockwise', middleFrame))
  }
}
