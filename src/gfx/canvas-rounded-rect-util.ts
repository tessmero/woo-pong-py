...
export function fillFrameBetweenRectAndRounded(
  ...
  const [x, y, w, h] = rect
  const r = Math.min(ROUNDED_RECT_RADIUS * window.devicePixelRatio, w / 2, h / 2)
...
export const ROUNDED_RECT_RADIUS = 8
export const ROUNDED_RECT_PADDING = 2
...
export function drawRoundedRect(
  ...
  ctx.quadraticCurveTo(x + pad, y + pad, x + pad + r, y + pad)
  ctx.closePath()
  ctx.fillStyle = isHovered ? '#aaa' : isActive ? ACTIVE_FILL_COLOR : ROUNDED_RECT_FILL_COLOR
  ctx.fill()
  ...
  if (!skipSlivers) {
  // --- Draw lit top sliver ---
    const sliverFrac = 0.15 // fraction of height for sliver thickness
    const sliverCurve = 5 * window.devicePixelRatio
    const sliverPad = pad + 1
    const sliverR = Math.max(r - 1, 2)
    const sliverH = Math.max(2, Math.floor((height - 2 * sliverPad) * sliverFrac))
    ctx.beginPath()

    // outer edge of sliver following main rounded rectangle shape
