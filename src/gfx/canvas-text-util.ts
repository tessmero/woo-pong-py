 ...
 * @param isBold Optional: use bold weight (default: false)
 */
export function setupRubikText(ctx: CanvasRenderingContext2D, h: number, color: string, isBold = false) {
  const scale = (h * 0.55) / BASE_FONT_SIZE;
  ctx.translate(0, 0);
  ctx.scale(scale, scale);
  ctx.font = `${isBold ? 700 : FONT_WEIGHT} ${BASE_FONT_SIZE}px Rubik, sans-serif`;
